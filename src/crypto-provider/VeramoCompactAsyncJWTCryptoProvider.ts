...
import { CreateCredentialOptions, CredentialPayload, VerifyCredentialOptions, createVerifiableCredentialJwt, verifyCredential as verifyCredentialJwt } from 'did-jwt-vc';
...
export class VeramoCompactAsyncJWTCryptoProvider implements AsyncJWTCryptoProvider {
    ...
    async signAsync(payload: JSONObject, keyId?: string | null | undefined, options?: VeramoCompactAsyncJWTCryptoProviderSignOptions): Promise<string> {
        // validate options
        if (!options) {
        ...
        // sign + return
        return await createVerifiableCredentialJwt(payload as CredentialPayload, {
            did: options.did,
    ...
    async verifyAsync(jwt: string, options?: VeramoCompactAsyncJWTCryptoProviderVerifyOptions): Promise<JWTVerificationResult> {
        // validate options
        if (!options) {
            throw new Error('invalid_argument: options must be provided');
        }
